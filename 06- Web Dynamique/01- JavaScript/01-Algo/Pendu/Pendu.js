...
function afficherTableau(tab) {
    ...
    document.getElementById("mot").innerHTML = texte;
}
...
function testerLettre(lettre, tab, depart) {
    tabRec = tab.slice(depart); // on détermine le tableau de recherche en fonction de la position depart.
    //Array_slice, permet d'extraire le sous tableau en fonction de la position
    // exemple array_slice(['B','O','N','J','O','U','R'], 5) donne ['U','R']
    res = tabRec.indexOf(lettre); //recherche la lettre dans le tableau
    if (res == -1) // === false pour eviter la confusion entre 0 et false
    {
        ...
        tabPos = reponse.concat(testerLettre(lettre, tab, res + depart + 1)); // array_merge permet de fusionner le tableau résultat avec le tableau de l'appel recursif
...
function ajouterLesLettres(val, tab, tabpos, niveau) {
    switch (niveau) {
        ...
        case 3:
            ...
            pos = tabpos.filter(x => test.indexOf(x) === -1); //différence entre les tableaux
            if (pos.length > 0)        //s'il reste des lettres à placer
            {
                posetudie = pos[Math.floor(Math.random() * (pos.length - 1))];    //on choisit une position au hasard
...
function afficherMauvaisesLettres(listeLettres) {
    var texte = "<br> Les lettres non présentes sont ";
    taille = listeLettres.length;
    for (i = 0; i < taille; i++) {
        if (i == taille - 1) // evite la , après la dernière lettre
        {
            texte += listeLettres[i] + "<br>";
        }
        else {
            texte += listeLettres[i] + ",";
        }
    }
    document.getElementById("mauvaiseLettre").innerHTML = texte;
}
...
function choisirMot(niv) {
    ...
    {
        do {
            nb = Math.floor(Math.random() * (dico.length - 1));
...
function demanderLettre() {
    do {
        //var lettre = prompt("entrez une lettre : ").toUpperCase(); // sans input
        var lettre = document.getElementById("reponse").value; // avec input
...
function testerGagner(nberreur, tab) {
    if (nberreur == 8) // si nb erreur =9, partie perdue
    {
        return -1;
    }
...
function choisirNiveau() {
    do {
        niveau = parseInt(prompt("\t\tNiveau de difficulé :\n \tFacile (1)\t Normal (2)\t Difficile (3)\t Court(4)\n :"));
...
function lancerPartie(niveau) {
    ...
    document.getElementById("motATrouver").innerHTML = motATrouver; //affichage du mot à trouver
    ...
    do {
        ...
        if (mauvaisesLettres.length > 0) { //s'il y a des mauvaises lettres, on les affiche
        ...
        if (lesPositions.length == 0) { //la lettre n'est pas dans le mot
            nbErreur++;
    ...
    if (gagne == 1) {
        alert("Bravo!! vous avez gagné. Le mot était " + motATrouver + "\n");
    }
    else {
        alert("Vous avez perdu. Le mot était " + motATrouver + "\n");
    }
}
...
/**
 * Permet de gérer la partie avec bouton
 *
 * @return void
 */
function lancerPartieBouton() {
    var lettre = document.getElementById("reponse").value.toUpperCase();
    var lesPositions = testerLettre(lettre, tabMotATrouver, 0); //on recupere toutes les positions de cette lettre dans le mot
    if (lesPositions.length == 0) { //la lettre n'est pas dans le mot
        nbErreur++;
        mauvaisesLettres.push(lettre);

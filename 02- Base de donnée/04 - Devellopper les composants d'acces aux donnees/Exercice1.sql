...
SELECT nomClient,FLOOR(DATEDIFF(now(),`dateEntreeClient` )/365) AS Ancienneté FROM clients;
SELECT `nomClient`,YEAR(now())-YEAR(`dateEntreeClient`) AS Ancienneté FROM `clients`
